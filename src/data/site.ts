export type SiteLink = {
  label: string;
  href: string;
  external?: boolean;
};

export type NavigationItem = {
  label: string;
  href?: string;
  external?: boolean;
  children?: SiteLink[];
};

export const siteMeta = {
  title: 'Calumet Park Memorial',
  description:
    'Calumet Park Memorial offers compassionate support, memorial services, and peaceful grounds honoring loved ones with care and dignity.'
};

export const topBar = {
  hours: 'Mon–Sat 9:00am–5:00pm',
  phone: '(219) 769-8803',
  address: '2305 W. 73rd Ave, Merrillville, IN'
};

export const navigation: NavigationItem[] = [
  {
    label: 'Recent Loss',
    children: [
      { label: 'Immediate Need', href: '/immediate-need/' },
      { label: 'Obituaries', href: 'https://obits.mycalumetpark.com', external: true },
      { label: 'Grief Counseling', href: '/grief-counseling/' }
    ]
  },
  {
    label: 'Our Services',
    children: [
      { label: 'Only at Calumet Park', href: '/only-at-cpc/' },
      { label: 'Burials, Cremation and More', href: '/services/' },
      { label: 'Veterans', href: '/veterans/' },
      { label: 'Pre-planning', href: '/pre-planning/' }
    ]
  },
  {
    label: 'Resources + Education',
    children: [
      { label: "Calumet Park's GraveFinder", href: '/gravefinder/' },
      { label: 'The Path To Salvation', href: '/path-to-salvation-interactive/' },
      { label: 'FAQs', href: '/faqs/' }
    ]
  },
  {
    label: 'About Us',
    children: [
      { label: 'About Calumet Park', href: '/about/' },
      { label: 'Virtually Tour CPC', href: '/virtual-tour/' },
      { label: 'Locations + Hours', href: '/contact/' }
    ]
  },
  { label: 'Shop Urns', href: 'https://mycalumetpark.com/shop-urns/', external: true },
  { label: 'Obituaries', href: 'https://obits.mycalumetpark.com', external: true },
  { label: 'Contact', href: '/contact/' }
];

export const hero = {
  eyebrow: 'Calumet Park Memorial',
  title: 'A peaceful place of remembrance and care.',
  description:
    'We honor every life with grace, guidance, and thoughtful planning. Our team supports families with memorial services, peaceful grounds, and compassionate resources.',
  primaryCta: { label: 'Start Pre-Planning', href: '/pre-planning/' },
  secondaryCta: { label: 'Speak With Our Team', href: '/contact/' }
};

export const ctaCards = [
  {
    title: 'Pre-Plan With Confidence',
    description:
      'Plan ahead with our caring counselors and ensure your wishes are honored with clarity and ease.',
    cta: { label: 'Explore Pre-Planning', href: '/pre-planning/' }
  },
  {
    title: 'Immediate Need Support',
    description:
      'We are available day and night to guide families through arrangements, paperwork, and memorial selections.',
    cta: { label: 'Get Immediate Help', href: '/immediate-need/' }
  },
  {
    title: 'Obituaries & Tributes',
    description:
      'Share memories, service details, and heartfelt tributes in a respectful space for your loved one.',
    cta: { label: 'View Obituaries', href: 'https://obits.mycalumetpark.com', external: true }
  }
];

export const servicesHighlight = {
  eyebrow: 'Services & Memorials',
  title: 'Traditional, modern, and personalized options.',
  description:
    'Choose from burial, cremation, and celebration of life services that reflect every family’s wishes and values.',
  items: [
    {
      title: 'Traditional Burial',
      description:
        'Grounds with mature trees, ceremonial chapels, and guidance through every detail.'
    },
    {
      title: 'Cremation Options',
      description:
        'Flexible memorial formats with columbarium spaces and curated tribute experiences.'
    },
    {
      title: 'Veterans Honors',
      description:
        'Dedicated spaces and ceremonial support to recognize those who served.'
    }
  ]
};

export const valuesSection = {
  eyebrow: 'Our Commitment',
  title: 'Guiding families with dignity and grace.',
  description:
    'For generations, Calumet Park Memorial has offered peaceful grounds, thoughtful services, and a steady presence in moments that matter most.',
  highlights: [
    'A serene memorial park with curated gardens and chapels.',
    'Dedicated counselors to support you before, during, and after services.',
    'Faithful stewardship of every detail, from planning to remembrance.'
  ]
};

export const events = {
  eyebrow: 'Community',
  title: 'Upcoming gatherings and remembrance events.',
  description:
    'Join our seasonal services and community programs designed to honor and uplift.',
  items: [
    {
      title: "Mother's Day Remembrance",
      date: 'May 10, 2026',
      location: 'Reflection Gardens',
      href: '/mothers-day/'
    },
    {
      title: 'Memorial Weekend Tribute',
      date: 'May 23, 2026',
      location: 'Veterans Plaza',
      href: '/memorial-weekend/'
    },
    {
      title: 'Light the Darkness 5K',
      date: 'September 19, 2026',
      location: 'Calumet Park Grounds',
      href: '/5k/'
    }
  ]
};

export const verse = {
  eyebrow: 'Words of Comfort',
  title: '“Blessed are those who mourn, for they shall be comforted.”',
  reference: 'Matthew 5:4'
};

export const contactSection = {
  eyebrow: 'We are here to help',
  title: 'Compassionate guidance, whenever you need it.',
  description:
    'Reach out to our counselors for planning support, service coordination, or answers to your questions.',
  ctas: [
    { label: 'Call (219) 769-8803', href: 'tel:12197698803' },
    { label: 'Schedule a Visit', href: '/contact/' }
  ]
};

export const contactPage = {
  metaTitle: 'Contact | Calumet Park Memorial',
  metaDescription:
    'Contact Calumet Park for planning support, location details, office hours, and direct assistance.',
  hero: {
    eyebrow: 'Contact',
    title: 'We are ready to help, whenever you need us.',
    description:
      'Speak with our team about immediate needs, pre-planning, or memorial service questions. We are here to guide you with care.',
    primaryCta: { label: 'Call (219) 769-8803', href: 'tel:12197698803' },
    secondaryCta: { label: 'Message Our Team', href: '#message-us' }
  },
  locationsSection: {
    eyebrow: 'Our Locations',
    title: 'Visit any Calumet Park location across Northwest Indiana.',
    description:
      'Find addresses, phone numbers, fax numbers, and hours of operation for our cemetery, funeral homes, and chapels.'
  },
  locations: [
    {
      name: 'Calumet Park Cemetery & Crematory',
      address: '2305 West 73rd Avenue, Merrillville, IN 46410',
      addressHref: 'https://maps.google.com/?q=2305+West+73rd+Avenue+Merrillville+IN+46410',
      phone: '219-769-8803',
      phoneHref: 'tel:12197698803',
      fax: '219-736-7409',
      hours: ['8:30am-4:30pm, Monday-Friday', '9:00am-1:00pm, Saturday']
    },
    {
      name: 'Rendina Funeral Home',
      address: '5100 Cleveland Street, Gary, IN 46408',
      addressHref: 'https://maps.google.com/?q=5100+Cleveland+St+Gary+IN+46408',
      phone: '219-980-1141',
      phoneHref: 'tel:12199801141',
      fax: '219-980-1142',
      hours: ['8:30am-4:30pm, Monday-Friday']
    },
    {
      name: 'Calumet Park Funeral Chapel & Crematory',
      address: '370 N. County Line Road, Hobart, IN 46342',
      addressHref: 'https://maps.google.com/?q=370+N+County+Line+Rd+Hobart+IN+46342',
      phone: '219-940-3791',
      phoneHref: 'tel:12199403791',
      fax: '219-940-9794',
      hours: ['8:30am-4:30pm, Monday-Friday']
    },
    {
      name: 'Calumet Park Funeral Chapel',
      address: '7535 Taft Street, Merrillville, IN 46410',
      addressHref: 'https://maps.google.com/?q=7535+Taft+St+Merrillville+IN+46410',
      phone: '219-736-5840',
      phoneHref: 'tel:12197365840',
      fax: '219-685-5128',
      hours: ['8:30am-4:30pm, Monday-Friday']
    }
  ],
  scheduleSection: {
    title: 'More comfortable speaking with us?',
    description:
      'Schedule a call at a time that works for you, or call our team now for immediate guidance.',
    ctas: [
      { label: 'Schedule a Call', href: '#schedule' },
      { label: 'Call Now (219) 769-8803', href: 'tel:12197698803' }
    ]
  },
  messageSection: {
    eyebrow: 'Not urgent? Message us!',
    title: 'Send us a note and a counselor will follow up shortly.',
    description:
      'Share what you need and we will connect you to the right team member for planning, services, or general support.',
    inquiryOptions: [
      'I want to pre-plan.',
      "I'd like to schedule a general meeting.",
      "I'd like more green burial information.",
      'I have a question about aerial ash scattering.'
    ]
  },
  mapSection: {
    title: 'Main Office',
    description: 'Calumet Park Cemetery & Crematory',
    address: '2305 West 73rd Avenue, Merrillville, IN 46410',
    mapEmbedUrl:
      'https://www.google.com/maps?q=2305+West+73rd+Avenue+Merrillville+IN+46410&output=embed'
  }
};

export const footer = {
  newsletter: {
    title: 'Keep in Contact',
    description: 'Seasonal programs, memorial events, and community gatherings.',
    placeholder: 'Enter your email'
  },
  columns: [
    {
      title: 'Products + Services',
      links: [
        { label: 'Traditional Burial Options', href: '/traditional-burial-options/' },
        { label: 'Popular Cremation Options', href: '/popular-cremation-options/' },
        { label: 'Resting Places', href: '/resting-places/' },
        { label: 'Custom Memorials', href: '/custom-memorials/' }
      ]
    },
    {
      title: 'New at CPC',
      links: [
        { label: "Angel's Flight", href: '/only-at-cpc/#angels-flight' },
        { label: 'Aerial Ash Scattering', href: '/only-at-cpc/#aerial-ash-scattering' },
        { label: 'Green Burial', href: '/only-at-cpc/#green-burial' },
        { label: 'Hike-In Ash Scattering', href: '/only-at-cpc/#hike-in-ash-scattering' },
        { label: 'Scattering Gardens', href: '/only-at-cpc/#scattering-gardens' }
      ]
    },
    {
      title: 'Helpful Resources',
      links: [
        { label: 'Schedule Your Visit', href: '/schedule-your-visit/' },
        { label: 'Locations + Hours', href: '/contact/' },
        { label: 'Veterans Benefits', href: '/veterans/' },
        { label: 'Work With Us', href: '/apply/' },
        { label: 'Privacy policy', href: '/privacy/' }
      ]
    }
  ],
  bottom: '© 2026 Calumet Park Memorial. All rights reserved.'
};
